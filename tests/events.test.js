...
test('イベント: 誤答はその場でやり直し(ペナルティなし)、正答だけ前進', () => {
  ...
  const r2 = run.answer(run.cur().entry.w);
...
test('イベント: 選択肢に正解がちょうど1つ・4択', () => {
  ...
  for (const s of run.steps.filter((x) => x.t === 'cast')) {
    ...
    assert.equal(s.choices.filter((c) => c.w === s.entry.w).length, 1);
  }
});
...
test('イベント: 反芻は既習語から抽選されSRSに書き込まない', () => {
  ...
  assert.ok(['apple', 'angry', 'always'].includes(reviews[0].entry.w));
  run.answer; // 走らせても…
  for (const s of run.steps) if (s.t === 'cast') { run.idx = run.steps.indexOf(s); run.answer(s.entry.w); }
