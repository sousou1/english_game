...
console.log('day | 語彙 | settled(S>=3) | tier平均 | 1日ダメージ | 討伐累計 | 次の敵HP');
for (let day = 1; day <= 21; day++) {
  ...
  for (let t = 0; t < 1260; t++) {
    ...
    dmg += CURVE.tapBase * mult * coll * combo * 1.2;    // 1.2=鮮度平均。武器/ラッシュは含めない(保守的)
  }
  totalDmg += dmg;
  ...
  console.log(`${String(day).padStart(2)} | ${String(cards.length + steps.length).padStart(3)} | ${String(settled).padStart(2)} | ${tierAvg} | ${dmg.toExponential(2)} | ${String(kills).padStart(3)} | ${enemyHp(kills).toExponential(2)}`);
