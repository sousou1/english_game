...
import { idleRate, manaCap, settle, productionAt, storageCap, dormCap, FACILITIES, facilityPrice, timeToFull, TIER_NAMES } from './economy.js';
...
export class Workshop {
  constructor(app) {
    ...
    this.boost = 0;          // ふいごのブースト(0..1.5、×(1+boost))
    this.boostTouchedAt = 0;
  ...
  tick(now = Date.now()) {
    ...
    if (this.boost > 0) {
      ...
      this.boost *= Math.pow(0.5, dt / 3);
      if (this.boost < 0.01) this.boost = 0;
    }
    this.lastTick = now;
    if (span >= 1000) {
      const res = settle(p, this.app.index, p.settledAt, now);
      // アクティブ中のふいご増幅: 精算が生んだ増分にのみ乗せる(ミントではなく増幅)
      const extra = res.gained * this.boost;
      const cap = storageCap(p);
      p.lights = Math.min(cap, res.lights + extra);
      p.totalLights += res.gained + Math.min(extra, Math.max(0, cap - res.lights));
  ...
  // ---- ふいご(クリッカー) ----
  // タップは灯火を発行しない。生産への乗算バフ(+8%/タップ、上限+150%)のみ
  fan(now = Date.now()) {
    this.boost = Math.min(1.5, this.boost + 0.08);
    this.boostTouchedAt = now;
    const { rate } = productionAt(this.app.profile, this.app.index, now);
    return { boost: this.boost, producing: rate > 0 };
  }
  ...
  snapshot(now = Date.now()) {
    const p = this.app.profile;
    const { rate } = productionAt(p, this.app.index, now, 1 + this.boost);
    return {
